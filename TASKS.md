...
- [ ] **Push des 9 commits locaux** (`03de953`..`4c06934`) + du travail PBI 013 (non commité) vers `origin/main` — attend validation explicite. Voir `PROJECT_STATE.md`.
...
- [ ] Conteneur Docker `marsai_backend` toujours cassé (`dist/server.js` manquant, image jamais reconstruite) — contournement : backend lancé en local (`npm run dev`) pointé sur MySQL Docker (port 3310) pour la vérification des PBI 010-013.
...
- [x] PBI 013 — page de succès de soumission (`/success`) : paragraphe de description codé en dur en français (page mi-anglaise mi-française en langue EN), corrigé ; ajout de `tests/submission-success.spec.ts` (contenu vérifié FR + EN), seule l'URL était vérifiée jusqu'ici (non commité)
