...
test.describe('Jury evaluator i18n', () => {
  test('translates the AI identity card and rating sections between FR and EN', async ({ page }) => {
    ...
    // Voted films show the locked label instead of the submit label (PBI 041).
    await expect(
      page.getByRole('button', { name: /Soumettre l'Évaluation|Vote déjà enregistré/ })
    ).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: /Submit Rating|Vote already recorded/ })).toBeVisible();
  });
});
