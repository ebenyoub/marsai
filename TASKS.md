...
- [ ] **Push des 12 commits locaux** (`03de953`..`a9309c5`) + du travail PBI 015 (non commité) vers `origin/main` — attend validation explicite. Voir `PROJECT_STATE.md`.
...
- [ ] Conteneur Docker `marsai_backend` toujours cassé (`dist/server.js` manquant, image jamais reconstruite) — contournement : backend lancé en local (`npm run dev`) pointé sur MySQL Docker (port 3310) pour la vérification des PBI 010-015.
...
- [x] PBI 015 — galerie d'accueil (`VideoSection.tsx`) sans traduction anglaise (titre, chargement, erreur, vide, compteur, placeholder de recherche) ; même défaut corrigé dans `JuryDashboard.tsx` ; ajout de `tests/gallery-i18n.spec.ts` (non commité)
- [x] Création de `ROADMAP_V2.md` : réinitialisation de mot de passe et pages légales du footer déplacées hors `PROJECT_STATE.md` avec justification (commit `a9309c5`)
